...
def vShape(gridshp): # make viirs shapefile
    ...
    layer = utcTolocal(clippedlayer, utc)
    layer.dataProvider().createSpatialIndex()
    
    layer.selectByExpression("\"date\">='{}' and \"date\"<='{}'".format(cycles[1], cycles[len(cycles)-2]))
...
def mShape(gridshp): # make modis shapefile 
...
def gridstats(gridshp, hshape, vshape, mshape): # calculate grid stats
    ...
    Path(results).joinpath('{}'.format(country), '{}'.format(date), '{}-day'.format(cycle), '{}x{}-grid'.format(n, n)).mkdir(parents=True, exist_ok=True)
    
    output = str(Path(results).joinpath(country, date, '{}-day'.format(cycle), '{}x{}-grid'.format(n, n))) + "\\" + "{}_{}_{}-day_{}x{}-grid_{}_{}_gridstats".format(country,cycles[1],cycle,n, n,lon,lat) + '.csv'
    options = QgsVectorFileWriter.SaveVectorOptions()
    options.driverName = "CSV"

    QgsVectorFileWriter.writeAsVectorFormatV2(hvmfrp, output, QgsCoordinateTransformContext(), options)
    
    output = str(Path(results).joinpath(country, date, '{}-day'.format(cycle), '{}x{}-grid'.format(n, n))) + "\\" + "{}_{}_{}-day_{}x{}-grid_{}_{}_gridstats".format(country,cycles[1],cycle,n, n,lon,lat) + '.shp'
    options = QgsVectorFileWriter.SaveVectorOptions()
...
def mergeStats(): # merge 3 layers
    ...
    output = str(Path(results).joinpath(country, date, '{}-day'.format(cycle), '{}x{}-grid'.format(n, n))) + "\\" + "{}_{}_{}-day_{}x{}-grid_{}_{}_mergestats".format(country,cycles[1],cycle,n, n,lon,lat) + '.csv'
    options = QgsVectorFileWriter.SaveVectorOptions()
    options.driverName = "CSV"

    QgsVectorFileWriter.writeAsVectorFormatV2(mergestats, output, QgsCoordinateTransformContext(), options)
    
    output = str(Path(results).joinpath(country, date, '{}-day'.format(cycle), '{}x{}-grid'.format(n, n))) + "\\" + "{}_{}_{}-day_{}x{}-grid_{}_{}_mergestats".format(country,cycles[1],cycle,n, n,lon,lat) + '.shp'
    options = QgsVectorFileWriter.SaveVectorOptions()
    options.driverName = "ESRI Shapefile"
...
utc = getUTC(lon, lat) 
cycles = cycles(date, cycle)

hshape = hShape(gridshp, hcon) 
vshape = vShape(gridshp) 
mshape = mShape(gridshp) 

country = getCountry(lon, lat)
results = r"D:\WORK September 2022\seag\data\results"
