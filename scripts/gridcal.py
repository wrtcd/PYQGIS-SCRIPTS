...
def hShape(gridshp, date, hcon): # make himawari shapefile 
    ...
    for coords in coordslist:
        ...
        for f in fires:
            ...
            data["HH:MM"] = pd.to_datetime(data.hours, format='%H%M')
            data['HH:MM'] = data['HH:MM'].map(lambda t: t.replace(year=year, month=month, day=day))
    ...
    newdf['HH:MM'] = newdf['HH:MM'] + pd.DateOffset(hours=utc)
    newdf['date'] = pd.to_datetime(newdf['HH:MM'].dt.date)
    newdf = newdf.loc[newdf['date'] == date]
    newdf = newdf.sort_values(['id', 'HH:MM', 'date'])
    newdf["HH:MM"] = newdf["HH:MM"].apply(lambda x: x.strftime("%I:%M %p"))
    newdf["date"] = newdf["date"].astype(str)
    newdf["HH:MM"] = newdf["HH:MM"].astype(str)
    newdf = newdf.rename(columns={'#year': 'year'})
    newdf = newdf.drop(['year', 'month', 'day', 'hour', 'hours'], axis=1)
    newdf = newdf[['lat','lon','ave(frp)','max(frp)','ave(confidence)','id','date','HH:MM','N']]
    ...
    hshape_provider.addAttributes([QgsField("HH:MM",QVariant.String, 'text')])
...
gridshp = createGrid(lat, lon, n) # create nxn grid with id, lat and long
coordslist = getCoordsList(lat, lon, n) # create list of grid cenroids i.e, points for which data is extracted
utc = getUTC(lon, lat) # get utc time for point 

hshape = hShape(gridshp, date, hcon) # himwari hourly intersecting grid for 1 complete day (local time)
vshape = vShape(gridshp, date) # viirs in grid
mshape = mShape(gridshp, date) # modis in grid
...
mergestats = mergeStats() # master file
gridstats = gridstats(gridshp, hshape, vshape, mshape) # create grid stats
